...
const managedScroll = ref(false);

const scroll = (amount = 1, behavior = 'smooth') => {
    managedScroll.value = true;
    page.value += amount;
...
const onScroll = () => {
    // We're scrolling using the button, cancel this or else stuff will jitter and skip
    if (managedScroll.value) return;
    page.value = Math.round(scroller.value.scrollLeft / scroller.value.clientWidth);
}
...
<template>
    <div class="flex items-center relative">
        <div v-if="page > 0" @pointerdown="scroll(-1)" class="absolute z-10 left-3 drop-shadow cursor-pointer text-white bg-neutral-700 rounded-full">
